...
export function MemoryWorkspace({ initialPath, onDirtyChange, active = true }: { initialPath?: string; onDirtyChange?: (dirty: boolean) => void; active?: boolean }) {
  ...
  const loadDocument = useCallback(async (path: string, force = false) => {
    ...
    try {
      ...
      if (window.innerWidth < 1024) requestAnimationFrame(() => window.document.getElementById("memory-document")?.scrollIntoView({ block: "start" }));
    } catch (error) {
  ...
  if (!catalog) return <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-700"><p>{message?.text ?? "Codex memory is unavailable."}</p><Button className="mt-3" variant="outline" onClick={loadCatalog}>Try again</Button></div>;

  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0"><div className="mb-2 flex items-center gap-2"><Badge className="border-cyan-200 bg-cyan-50 text-cyan-700"><Brain className="size-3" />Markdown corpus</Badge><span className="truncate text-xs text-muted-foreground">{catalog.root}</span></div><h1 className="text-2xl font-semibold tracking-[-0.035em] sm:text-3xl">Codex Memory</h1><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Find remembered decisions, read their context, and edit Markdown files.</p></div>
      ...
      <div className="grid min-w-0 gap-5">
        <ModuleSidebar active={active}><Card className="min-w-0 overflow-hidden lg:flex lg:h-full lg:min-h-0 lg:flex-col lg:rounded-none lg:border-0 lg:bg-[#17202d] lg:text-white lg:[&_.text-muted-foreground]:text-slate-400">
          <CardHeader className="border-b p-4 lg:border-white/10"><CardTitle className="text-sm">Memory files</CardTitle><CardDescription>Every Markdown file under the memory root</CardDescription><div className="relative pt-2"><Search className="absolute left-3 top-[26px] size-3.5 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Search Memory contents" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search all contents…" className="sidebar-form-border pl-9 pr-8 lg:bg-white/[0.06] lg:text-white lg:placeholder:text-slate-400" />{query && <button aria-label="Clear Memory search" onClick={() => setQuery("")} className="absolute right-3 top-[26px] -translate-y-1/2 text-muted-foreground"><X className="size-3.5" /></button>}</div>{!query && <select aria-label="Memory directory" value={directory} onChange={(event) => setDirectory(event.target.value)} className="sidebar-form-border mt-2 h-8 w-full rounded-lg border bg-white px-2 text-xs lg:bg-white/[0.06] lg:text-slate-200 lg:[&>option]:bg-white lg:[&>option]:text-slate-900"><option value="All">All directories ({catalog.files.length})</option>{catalog.directories.map((item) => <option key={item} value={item}>{item} ({directoryCounts[item]})</option>)}</select>}</CardHeader>
        ...
        <Card id="memory-document" className="min-w-0 scroll-mt-20 overflow-hidden">
          {document ? <Fragment><div className="flex flex-col gap-3 border-b px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-sm font-semibold">{document.title}</p>{dirty && <Badge variant="warning">unsaved</Badge>}</div><p className="truncate font-mono text-[10px] text-muted-foreground">{document.path}</p></div><div className="flex items-center gap-2"><div className="flex rounded-lg border bg-muted/40 p-0.5"><button aria-pressed={view === "edit"} onClick={() => setView("edit")} className={cn("flex h-7 items-center gap-1 rounded-md px-2 text-[11px]", view === "edit" ? "bg-white font-medium shadow-sm" : "text-muted-foreground")}><Pencil className="size-3" />Edit</button><button aria-pressed={view === "preview"} onClick={() => setView("preview")} className={cn("flex h-7 items-center gap-1 rounded-md px-2 text-[11px]", view === "preview" ? "bg-white font-medium shadow-sm" : "text-muted-foreground")}><Eye className="size-3" />Preview</button></div><Button size="sm" onClick={save} title="Save (⌘ / Ctrl + S)" disabled={!dirty || saving || documentLoading}>{saving ? <Loader2 className="size-3.5 animate-spin" /> : <Save className="size-3.5" />}Save</Button>{dirty && <Button variant="ghost" size="sm" disabled={saving} onClick={() => { if (window.confirm("Discard the unsaved changes in this memory file?")) { setEditedContent(document.content); setMessage(null); } }}>Discard</Button>}</div></div>{documentLoading ? <div className="flex min-h-[600px] items-center justify-center gap-2 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" />Opening file…</div> : view === "edit" ? <textarea aria-label="Memory Markdown" readOnly={saving} onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "s") { event.preventDefault(); void save(); } }} value={editedContent} onChange={(event) => setEditedContent(event.target.value)} spellCheck={false} className="scrollbar-thin min-h-[420px] sm:min-h-[600px] w-full resize-y bg-[#151c27] p-5 font-mono text-[12px] leading-6 text-slate-200 outline-none" /> : <div role="region" aria-label="Memory preview" tabIndex={0} className="scrollbar-thin min-h-[420px] sm:min-h-[600px] max-h-[760px] overflow-y-auto bg-white"><MarkdownPreview content={editedContent} onForgetLine={document.path === "memory_summary.md" && !dirty ? previewForget : undefined} /></div>}<div className="flex flex-wrap items-center justify-between gap-2 border-t px-4 py-2 text-[10px] text-muted-foreground"><span>{formatNumber(editedContent.length)} characters · {formatNumber(editedContent.match(/[\p{L}\p{N}][\p{L}\p{N}_'-]*/gu)?.length ?? 0)} words</span><span>Loaded {formatDate(document.modifiedAt)} · rev {document.hash.slice(0, 8)}</span></div></Fragment> : <div className="flex min-h-[420px] sm:min-h-[600px] items-center justify-center text-sm text-muted-foreground">{catalog.files.length ? "Select a memory file." : "No Markdown files found in this Memory folder."}</div>}
        </Card>
