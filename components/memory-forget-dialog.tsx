...
type Props = {
  ...
  project?: { directory: string; onDirectoryChange: (directory: string) => void; onPreview: () => void };
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!loading) onOpenChange(next); }}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        ...
        {project && <form className="space-y-2" onSubmit={(event) => { event.preventDefault(); project.onPreview(); }}><label htmlFor="forget-project-directory" className="text-xs font-semibold">Project directory</label><Input id="forget-project-directory" list="forget-project-scopes" value={project.directory} onChange={(event) => project.onDirectoryChange(event.target.value)} placeholder="/absolute/project/directory" disabled={loading} autoComplete="off" /><datalist id="forget-project-scopes">{projectPlan?.knownProjectScopes.map((scope) => <option key={scope} value={scope} />)}</datalist><Button type="submit" variant="outline" size="sm" disabled={loading || !project.directory.trim()}><RefreshCw className={cn("size-3.5", loading && "animate-spin")} />{visiblePlan && project.directory ? "Refresh preview" : "Preview project"}</Button></form>}
        ...
        {project && <div className="flex justify-end"><Button variant="outline" onClick={() => onOpenChange(false)} disabled={loading}>Cancel</Button></div>}
      </DialogContent>
    </Dialog>
  );
}
