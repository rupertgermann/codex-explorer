...
function Sections({ title, sections }: { title: string; sections: ForgetSection[] }) {
  return <div className="space-y-2"><p className="text-xs font-semibold">{title} ({sections.length})</p>{sections.map((section) => <div key={section.id} className="rounded-lg border p-3 text-xs"><div className="flex items-center justify-between gap-2"><Badge variant="secondary">{section.kind}</Badge><span className="font-mono text-[11px] text-muted-foreground">{section.path}:{section.startLine}-{section.endLine}</span></div><pre className="mt-2 whitespace-pre-wrap font-sans text-[11px] text-muted-foreground">{section.content.trim()}</pre></div>)}</div>;
}
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!loading) onOpenChange(next); }}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto [overflow-wrap:anywhere]">
        ...
        {loading && !plan ? <div role="status" className="flex items-center justify-center gap-2 p-8 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" />Building read-only preview…</div> : null}
        ...
        {visiblePlan && <div className="space-y-4">
          ...
          {needsSourceConfirmation && summaryPlan && !result && <div className="space-y-2">
            ...
            {summaryPlan.durableCandidates.map((candidate) => <label key={candidate.id} className="flex gap-2 rounded-lg border p-3 text-xs"><input type="checkbox" className="mt-0.5 size-4 shrink-0 accent-primary" disabled={loading} checked={confirmedDurableIds.includes(candidate.id)} onChange={(event) => onConfirm(candidate.id, event.target.checked)} /><span><b>{candidate.path}:{candidate.startLine}</b><span className="mt-1 block text-muted-foreground">{candidate.content.trim()}</span>{candidate.signals?.length ? <span className="mt-1 block text-[11px] text-accent-foreground">Matched by {candidate.signals.join(", ")}</span> : null}</span></label>)}
          ...
          {result && <div className="space-y-2 rounded-lg border border-emerald-200 bg-emerald-50 dark:border-emerald-400/25 dark:bg-emerald-400/10 p-3 text-xs text-emerald-800 dark:text-emerald-200"><p className="font-semibold">Memory removed and verified; delete tombstone written.</p><p>Tombstone: {result.tombstonePath}</p><div><p className="font-medium">Changed artifacts ({result.changedPaths.length})</p><ul className="mt-1 list-disc pl-4 font-mono text-[11px]">{result.changedPaths.map((path) => <li key={path}>{path}</li>)}</ul></div><p className="break-all font-mono text-[11px]">Backup: {result.manifestPath}</p></div>}
          {result?.removedDatabaseRows !== undefined && <p className="text-xs text-emerald-800 dark:text-emerald-200">{result.removedDatabaseRows} database Memory rows removed. No targeted positive Memory remains in Markdown or stage1_outputs.</p>}
          {recheck && <div className={cn("rounded-lg border p-3 text-xs", recheck.status === "suppressed" ? "border-emerald-200 bg-emerald-50 dark:border-emerald-400/25 dark:bg-emerald-400/10 text-emerald-800 dark:text-emerald-200" : "border-amber-200 bg-amber-50 dark:border-amber-400/25 dark:bg-amber-400/10 text-amber-800 dark:text-amber-200")}>{recheck.status === "suppressed" ? "No positive copy currently appears in the Memory corpus." : `Memory resurfaced in ${recheck.resurfaced.map(({ path }) => path).join(", ")}.`}</div>}
          {!project && <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => onOpenChange(false)} disabled={loading}>Close</Button>{result ? <Button onClick={onRecheck} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}Recheck now</Button> : <Button variant="destructive" onClick={onApply} disabled={!visiblePlan.actionable || loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}Apply Forget plan</Button>}</div>}
        </div>}
